...
|QMS(품질관리)|품질관리, 품질계획관리|품절팀|
|CMMS|설비 유지보수|설비팀|

- 공정관리
    - MES의 한 파트인 공정(MRP: 자재 소요 계획)을 실시간으로 모니터링, 제어
    - 스마트팩토리로 실시간으로 양품 및 불량 선별 데이터 생성
...
- 확장핀 1 - PWM 확장핀, 5V,D6, D6, GND, D3(A와 공유)
...
- 기어드 DC 모터 제어
    - 0 ~ 255 사이에서 제어, 실제 50이하는 동작X
    - Default 80
    - 10부터 시작하면 60에서도 동작 안함.255부터 줄여가면 50까지 동작

- Serial Monitor 사용 주의점
    - 시리얼 입력에서 new Line, Carrige Return 선택, 입력 시 다른값이 입력됨
...
- Arduino UNO R3에서 Arduino UNO R4로 교체
